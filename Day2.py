...
#Idea general --> Obtenemos las reglas que hacen una contraseña válida e iteramos
#* Nota --> Tb se podría hacer usando splitted, pero yo en el momento no lo conocía

def ObtenerDatos(passw):
    ...
    while passw[it2] != ' ':
        it2 += 1
    cota_max = passw[it+1:it2]
    
    #Sacamos la letra
    letra = passw[it2+1]
    
    return {"min": cota_min, "max": cota_max, "letra": letra, "inicio": it2+1}
    
    
def contarClavesPol1(input):
    
    datos = {}
    total = 0
    
    for i in range(len(input)):
        datos = ObtenerDatos(input[i])
        cont = 0
        for j in range(datos["inicio"]+1, len(input[i])):
            if (input[i][j] == datos["letra"]):
                cont+=1
        if(cont >= (int)(datos["min"]) and cont <= (int)(datos["max"])):
            total += 1
    
    return total

#print(contarClavesPol1(input))

#SEGUNDA PARTE
#-------------

def contarClavesPol2(input):
    
    datos = {}
    total = 0
    trash = 2       #Espacios y otros caracteres no relevantes
    
    for i in range(len(input)):
        datos = ObtenerDatos(input[i])
        primero = (int) (datos["min"]) + (int) (datos["inicio"]) + trash
        segundo = (int) (datos["max"]) + (int) (datos["inicio"]) + trash
        
        if( (input[i][primero] == datos["letra"]) != (input[i][segundo] == datos["letra"]) ):
            total += 1
            
    return total

print(contarClavesPol2(input))